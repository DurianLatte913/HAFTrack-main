...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='ostrack', choices=['ostrack', 'haftrack', 'ostrack_twobranch'],
...
if __name__ == "__main__":
    ...
    if args.script == "ostrack":    # 单模态测速
    ...
    elif args.script == "haftrack":    # 多模态测速
        
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_haftrack
        model = model_constructor(cfg, training=False)
        model.eval()
        # get the template and search
        template = torch.randn(6, bs, 3, z_sz, z_sz)
        ...
        merge_layer = cfg.MODEL.BACKBONE.MERGE_LAYER
        if merge_layer <= 0:        # this is seleted
            evaluate_vit(model, template, search)
        else:
            evaluate_vit_separate(model, template, search)
            
    elif args.script == "ostrack_twobranch":    # 多模态测速
        
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_ostrack_twobranch
        model = model_constructor(cfg, training=False)
        # get the template and search
