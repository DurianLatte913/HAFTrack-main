...
"""
Add default config for CAiATrack.
"""
cfg = edict()
...
cfg.MODEL.BACKBONE.HCM_WAY = None  

cfg.MODEL.BACKBONE.CTE_LOC = []
cfg.MODEL.BACKBONE.CTE_KEEP_RATIO = []
cfg.MODEL.BACKBONE.CTE_TEMPLATE_RANGE = 'ALL'  # choose between ALL, CTR_POINT, CTR_REC, GT_BOX
cfg.MODEL.BACKBONE.JSLOSS_LOC = []
...
cfg.TRAIN.CTE_START_EPOCH = 20  # candidate elimination start epoch
cfg.TRAIN.CTE_WARM_EPOCH = 80  # candidate elimination warm up epoch
cfg.TRAIN.DROP_PATH_RATE = 0.1  # drop path rate for ViT backbone

# TRAIN.SCHEDULER
cfg.TRAIN.SCHEDULER = edict()
cfg.TRAIN.SCHEDULER.TYPE = "NULL"
cfg.TRAIN.SCHEDULER.DECAY_RATE = 0.1
cfg.TRAIN.SCHEDULER.LR_MIN = 0
cfg.TRAIN.SCHEDULER.WARMUP_T = 3
cfg.TRAIN.SCHEDULER.WARMUP_LR_INIT = 0
...
def update_config_from_file(filename, base_cfg=None):
    ...
    with open(filename) as f:
        ...
        print("从yaml加载的配置:", exp_config)  # 打印yaml中的配置
        if base_cfg is not None:
            _update_config(base_cfg, exp_config)
            print("更新后的base_cfg:", base_cfg)  # 打印更新后的配置
        else:
            _update_config(cfg, exp_config)
            print("更新后的全局cfg:", cfg)  # 新增：打印全局cfg的更新结果
