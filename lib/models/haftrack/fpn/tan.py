...
class Attention(nn.Module):
    def __init__(self, dim_q, dim_kv, num_heads=4, qkv_bias=False, stride=1, attn_drop=0., proj_drop=0.):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(self, dim_q, dim_kv, cross=True, num_heads=4, mlp_ratio=2., qkv_bias=False, drop=0., attn_drop=0.,
                drop_path=0.,act_layer=nn.GELU, norm_layer=nn.LayerNorm, stride=1):
        super().__init__()
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm1 = norm_layer(dim_q)
        self.attn = Attention(
            dim_q, dim_kv,
            num_heads=num_heads, qkv_bias=qkv_bias, stride=stride, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward_cross(self, x, y):
        x = x + self.drop_path(self.attn(self.norm1(x), self.norm3(y)))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class TReFPN(nn.Module):
    ...
    def __init__(self, in_dim, num_blocks=1, pre_conv=None, **kwargs):
        super().__init__()
        ...
        if pre_conv is not None:
            self.bottleneck = nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(in_dim[i], pre_conv[i], kernel_size=1),  # 原卷积层
                    nn.BatchNorm2d(pre_conv[i]),
                    nn.ReLU(inplace=True),
                    nn.Dropout2d(p=0.2),  # 添加卷积层专用的dropout
                ) 
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            # 恢复空间维度：假设 N = H * W（例如 H=8, W=8）
            B, N, C = x[i].shape
            H = W = int(N ** 0.5)  # 需确保 N 是平方数，或根据实际尺寸计算
            x_4d = x[i].permute(0, 2, 1).view(B, C, H, W)  # 从 [B, N, C] 转换为 [B, C, H, W]
            feat.append(x_4d)
        feat = [self.bottleneck[i](feat[i]) for i in range(self.num_layers)]
        feat_l4, feat_l3, feat_l5 = feat    
