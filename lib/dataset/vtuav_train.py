...
class VTUAV(BaseVideoDataset):
    ...
    def _get_frame_path_norm(self, seq_path, modality, frame_id):
        seq_name = seq_path.split('/')[-1] 
        if seq_name in self.init_idx:
            init_idx = self.init_idx[seq_name]
        else:
            init_idx = 0
        nz = 6
        return os.path.join(seq_path, modality, str(frame_id+init_idx).zfill(nz)+'.jpg')    # frames start from 1
    
    def _get_frame(self, seq_path, modality, frame_id, norm=False):
        if norm == False:
            return self.image_loader(self._get_frame_path(seq_path, modality, frame_id))
        else:
            return self.image_loader(self._get_frame_path_norm(seq_path, modality, frame_id))
    ...
    def get_frames(self, seq_id, frame_ids, anno=None, norm=False):
        ...
        if self.modality in ['RGB']:
            ...
            seq_path = self._get_sequence_path(seq_id)
            seq_path_i = self._get_sequence_path(seq_id)
            frame_list_v = [self._get_frame(seq_path, 'rgb', f_id, norm) for f_id in frame_ids]
            frame_list_i = [self._get_frame(seq_path_i,'ir', f_id, norm) for f_id in frame_ids]
        ...
        for key, value in anno.items():
            len = value.shape[0]
            if norm:
                anno_frames[key] = [value[frame_ids[0]]]
            else:
