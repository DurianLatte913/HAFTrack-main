from lib.models.haftrack.block import HAF_Block
import torch
...
class ReadModalContribution:
    def __init__(self, net:HAF_Block, length=300) -> None:
        ...
        idxs = [i for i,blk in enumerate(net.backbone.blocks) if (isinstance(blk, HAF_Block) or isinstance(blk, HAF_Block))]
        for i in idxs[:-1]:
