...
import time  # 新增：用于生成时间戳，避免文件名重复
...
# ===================== 新增：全局保存配置 =====================
# 可视化结果保存根目录（可根据自己需求修改）
VIS_SAVE_ROOT = "/opt/data/VTUAV/HAFTrack-main/Vis_result"
# 确保保存目录存在
if not os.path.exists(VIS_SAVE_ROOT):
    os.makedirs(VIS_SAVE_ROOT)
...
def index_to_color(idx):
    return {
        ...
        1: cv_colors.RED.value,
        2: cv_colors.MINT.value,
        3: cv_colors.BLUE.value,
        4: cv_colors.BLACK.value,
        5: cv_colors.PURPLE.value,
        6: cv_colors.YELLOW.value,
    }[idx]
...
class VisImage(VisBase):
    def __init__(self, visdom, show_data, title):
        ...
        self.save_dir = "/opt/data/VTUAV/HAFTrack-main/Vis_result"
    ...
    def draw_data(self):
        ...
        self._save_image_to_local()

    def _save_image_to_local(self):
        """将tensor格式的图片保存到本地（私有方法）"""
        # 处理tensor：CPU -> 转numpy -> 调整维度（CxHxW → HxWxC）
        img_tensor = self.raw_data.clone().cpu()  # 转到CPU
        img_np = img_tensor.numpy().transpose(1, 2, 0)  # 维度转换：CxHxW → HxWxC
        
        # 处理数值范围：确保是0-255的uint8（Visdom的image张量通常是0-255的float）
        img_np = img_np.astype(np.uint8)
        
        # 颜色通道转换：RGB → BGR（cv2保存图片默认是BGR格式）
        img_np = cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)
        
        # 生成保存文件名（结合title，替换特殊字符，避免报错）
        # 示例：title是"Tracking-001" → 文件名"Tracking_001.jpg"
        safe_title = self.title.replace("-", "_").replace(" ", "_").replace(":", "_")
        save_path = os.path.join(self.save_dir, f"{safe_title}.jpg")
        
        # 避免文件名重复（可选：添加时间戳/序号）
        # 如果你需要按帧保存不覆盖，可改用：
        # import time
        # save_path = os.path.join(self.save_dir, f"{safe_title}_{int(time.time())}.jpg")
        
        # 保存图片
        cv2.imwrite(save_path, img_np)
        print(f"图片已保存到：{save_path}")  # 可选：打印保存路径，方便调试
...
class VisTracking(VisBase):
    ...
    def draw_data(self):
        ...
        resize_factor = 1
        if max(disp_image.shape) > 1200:
            resize_factor = 1200.0 / float(max(disp_image.shape))
        ...
        # 获取单边图像的宽度（因为是左右拼接，所以总宽度的一半就是偏移量）
        half_width = disp_image.shape[1] // 2

        for i, disp_rect in enumerate(boxes):
            color = index_to_color(i % 7)
            
            # 获取原始坐标 (对应左侧 RGB 图)
            x1, y1 = int(disp_rect[0]), int(disp_rect[1])
            x2, y2 = int(disp_rect[0] + disp_rect[2]), int(disp_rect[1] + disp_rect[3])
            
            # 1. 在左侧 (RGB) 画框
            cv2.rectangle(disp_image, (x1, y1), (x2, y2), color, 2)
            
            # 2. 在右侧 (TIR) 画框 (X轴平移 half_width)
            cv2.rectangle(disp_image, (x1 + half_width, y1), (x2 + half_width, y2), color, 2)

            # 绘制置信度分数（如果有的话，左右两边都标上）
            if scores is not None:
                score_str = "{:.3f}".format(scores[i])
                # 把文字往上移一点 (y1 - 5)，防止挡住框的边缘，并将颜色和框的颜色统一
                cv2.putText(disp_image, score_str, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
                cv2.putText(disp_image, score_str, (x1 + half_width, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
        for i, mask in enumerate(self.raw_data[2], 1):
            disp_image = overlay_mask(disp_image, mask * i)
        
        # ===================== 核心修改：更稳健的保存逻辑 =====================
        write_img = disp_image.copy()
        write_img = write_img[:, :, ::-1]  # RGB → BGR
        
        # 优先使用显式传入的 seq_name 和 frame_id 
        if len(self.raw_data) > 3:
            kwargs = self.raw_data[3]
            seq_name = kwargs.get('seq_name', 'unknown_seq')
            frame_id = kwargs.get('frame_id', self.count)
            should_save = kwargs.get('save_img', False)
            save_name = f"{seq_name}_{frame_id:04d}.jpg"
        else:
            # 兜底逻辑
            save_name = f"tracking_{self.count:04d}.jpg"
        if should_save:
            # 确保目录存在（按序列分文件夹存放会更整洁）
            seq_save_dir = os.path.join(VIS_SAVE_ROOT, seq_name)
            ...
            # 写入并验证
            success = cv2.imwrite(save_path, write_img)
            if not success:
                print(f"写入图片失败，请检查路径权限: {save_path}")
            
            self.count += 1
        
        # ===================== Visdom 前端显示逻辑 =====================
        # 只有真正开启了 show_data 时，才把 tensor 发送给 visdom
        if self.show_data: 
            # 1. 提取我们之前传进来的 caption 作为图片顶部的标题
            disp_title = self.title
            if len(self.raw_data) > 3 and 'caption' in self.raw_data[3]:
                disp_title = self.raw_data[3]['caption']

            # 2. 转换格式 (C, H, W)
            disp_image_tensor = numpy_to_torch(disp_image).squeeze(0).float()
            
            # 3. 💥 真正的发送命令
            self.visdom.image(
                disp_image_tensor, 
                opts={'title': disp_title}, 
                win=self.title  # 发送到 'Tracking' 窗口
            )
