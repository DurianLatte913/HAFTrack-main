...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                ...
                data = TensorDict({'template_images': template_frames,      # length=2*num_template_frames
                                   'template_anno': template_anno['bbox'],  # length=1*num_template_frames
                                   'template_masks': template_masks,        # length=1
                                   'search_images': search_frames,          # length=2
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            # -------------------------- 核心修改：适配VTUAV仅标注帧在seq_info_dict中的场景 --------------------------
            if is_video_dataset and dataset.name == "VTUA":
                # 步骤1：获取序列总帧数（必须从dataset中获取，因为seq_info_dict仅含标注帧）
                # 注：需确保dataset有get_seq_length方法，若没有则替换为实际获取总帧数的逻辑（如读取帧文件夹长度）
                total_frames = dataset.get_seq_length(seq_id)
            
                # 步骤2：从seq_info_dict中提取标注帧索引（仅标注帧在visible中为True）
                # seq_info_dict['visible']是Tensor，shape=[总帧数]，仅标注帧为True
                original_visible = seq_info_dict['visible']  # shape=[total_frames]
                anno_frame_ids = torch.where(original_visible)[0].tolist()  # 提取所有标注帧索引
            
                # 步骤3：构造目标帧集合（标注帧 + 每个标注帧的前2帧）
                target_frame_ids = set()
                for anno_idx in anno_frame_ids:
                    # 添加标注帧（硬标签）
                    if 0 <= anno_idx < total_frames:
                        target_frame_ids.add(anno_idx)
                    # 添加前2帧（软标签）：anno_idx-1、anno_idx-2
                    for offset in [1, 2]:
                        soft_idx = anno_idx - offset
                        if 0 <= soft_idx < total_frames:  # 仅需索引有效（无需检查original_visible，因为原visible仅标注帧为True）
                            target_frame_ids.add(soft_idx)
            
                # 步骤4：构造完整的visible张量（覆盖所有帧，仅目标帧为True）
                visible = torch.zeros(total_frames, dtype=torch.bool)  # 初始化全False
                for idx in target_frame_ids:
                    visible[idx] = True
            
                # 更新seq_info_dict中的visible（保证后续流程使用新的visible）
            ...
            # 2. 调整“足够可见帧”的判断条件（适配VTUAV筛选后的数据）
            if is_video_dataset and dataset.name == "VTUA":
                # VTUAV：筛选后只需保证可用帧数量 > 搜索帧+模板帧数量（无需2倍，因为筛选后数量少）
                enough_visible_frames = (
                    visible.type(torch.int64).sum().item() > (self.num_search_frames + self.num_template_frames) 
                    and len(visible) >= 20  # 保留原有的序列长度要求
                )
            else:
